...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
  float calculated_H_value;
  calculated_H_value = node->distance((*end_node));
  return calculated_H_value;
}
...
// - Return the pointer.
bool Compare(RouteModel::Node *first_neighbor,
             RouteModel::Node *other_neighbor) {
  float gh_value_fisrt_neighbor;
  float gh_value_other_neighbor;

  gh_value_fisrt_neighbor = first_neighbor->g_value + first_neighbor->h_value;
  gh_value_other_neighbor = other_neighbor->g_value + other_neighbor->h_value;

  return gh_value_fisrt_neighbor > gh_value_other_neighbor;
}

RouteModel::Node *RoutePlanner::NextNode() {

  sort(open_list.begin(), open_list.end(), Compare);
  RouteModel::Node *lowst_sum_ptr = open_list.back();
  open_list.pop_back();
  return lowst_sum_ptr;
}
